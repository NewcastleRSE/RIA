...
3. It is also necessary to having working copies of the programs `PLINK <https://www.cog-genomics.org/plink/>`_ and `KING version 2.2.9 <https://www.kingrelatedness.com/history.shtml>`_ **Note:** It is very important to download KING version 2.2.9 and not any later versions as we need to use the `--homog` option which is no longer available in later versions. 
